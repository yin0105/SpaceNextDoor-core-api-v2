...
import { ISiteAddressEntity } from './interfaces/site-address.interface';
import { SiteAddressModel } from './site-address.model';

@Resolver('SiteAddress')
export class SiteAddressResolver {
  constructor(
  ...
  async country(@Parent() address: ISiteAddressEntity): Promise<Country> {
  ...
  @ResolveField('city')
  async city(@Parent() address: SiteAddressModel): Promise<City> {
    if (!address.city_id) {
      return null;
    }
  ...
  async district(@Parent() address: SiteAddressModel): Promise<District> {
